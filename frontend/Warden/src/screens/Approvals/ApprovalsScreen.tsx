...
function passToApproval(p: Pass): ApprovalRequest {
  return {
    id: p.passID,
    name: p.student?.Roll_No ?? p.RollNo, // student name comes from user lookup; use rollNo as fallback
    rollNo: p.RollNo,
...
export default function ApprovalsScreen() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setHostelId(hid);
      if (hid) {
        const data = await getApprovalsByHostel(hid);
        setPasses(data);
      }
    } catch (e: any) {
  ...
  return (
    <AppShell activeTab="approvals">
      <HeroHeader
        ...
        subtitle="Review and manage pending gatepass requests."
